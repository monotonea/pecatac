...
class kndr_radialBlurUI
{
	idd = KNDR_RADIAL_DLG;
	movingEnable = 1;
	controlsBackground[] = { };	
	name = "kndr radialblur preview UI";
	class controls
	{
		//-------------------------------------------
		...
		{
			action = " ""RadialBlur"" ppEffectEnable true; hint ""enable""; ";
			x = 0.8;
			y = 0.2;
			text = "enable";
		};
		...
		{
			action = " ""RadialBlur"" ppEffectEnable false; hint ""disable"";";
			x = 0.8;
			y = 0.25;
			text = "disable";
		};
		...
		class ASldr : kndrSlider
		{
			...
			x = 0.1 + (3 * KNDR_OFFSET);
			onSliderPosChanged = "rb1 = ((_this select 1) / 10); [] call setRadialBlur;"; 
		};
		class BSldr : kndrSlider
		{
			idc = KNDR_SLIDER0_ID + 4;
			x = 0.1 + (4 * KNDR_OFFSET);
			onSliderPosChanged = "rb2 = ((_this select 1) / 10); [] call setRadialBlur;"; 
		};
		class CSldr : kndrSlider
		{
			idc = KNDR_SLIDER0_ID + 5;
			x = 0.1 + (5 * KNDR_OFFSET);
			onSliderPosChanged = "rb3 = ((_this select 1) / 20); [] call setRadialBlur;"; 
		};
		class DSldr : kndrSlider
		{
			idc = KNDR_SLIDER0_ID + 6;
			x = 0.1 + (6 * KNDR_OFFSET);
			onSliderPosChanged = "rb4 = ((_this select 1) / 20); [] call setRadialBlur;"; 
		};
	};
};

...
if (isServer) then {
  ...
  while {_looking} do {
	...
	if (count _targets > 0) then
	{
		...
		while { (_count < count _targets) } do
		{
			...
			if ((_unit knowsAbout (_selectTarget select 4) > 0) && (_selectTarget select 2 == _side) && (_selectTarget select 4 isKindOf "Land")) then 
			{
 ...
 while {alive _unit && (_unit distance _target > 15)} do {
  ...
  // We're in range, make sure we're still alive then pull the pin!
	...
	"Bomb_04_F" createVehicle getPos _unit;"Bomb_04_F" createVehicle getPos _unit;"Bomb_04_F" createVehicle getPos _unit;
	deletevehicle _unit;
};

...
player addAction [" - Request Fire support -","arty_script.sqf", [], 1.5, true, true, "", "backpack player == 'gac_JGSDF_Backpack3'"];
...
ARTY_POS_RANDOM		= 90;	//ランダムな位置に着弾(0でも可)
	missionnamespace setvariable ["EW",117];	//初期座標(ヨコ)
	missionnamespace setvariable ["SN",066];	//初期座標(タテ)
player execVM "arty_gui\arty_player.sqf";
if (isServer) then{
	missionnamespace setvariable ["Arty_Ammo1",20];	//榴弾
	publicvariable "Arty_Ammo1";
	missionnamespace setvariable ["Arty_Ammo2",10];//煙幕弾

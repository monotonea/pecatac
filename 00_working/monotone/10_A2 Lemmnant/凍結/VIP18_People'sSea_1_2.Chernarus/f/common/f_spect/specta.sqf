...
CheckNewUnits = {
			...
			// Avoid game logics
			_foo = [];
			{if(side _x != sideLogic) then {_foo set [(count _foo) ,_x]}} foreach KEGsAllUnits; // {if(side _x != sideLogic) then {_foo = _foo + [_x]}} foreach KEGsAllUnits;
			
			{_allUnits set [(count _allUnits) ,(crew _x) select 0];_allVehicles set [(count _allVehicles), _x]} foreach _foo; // {_allUnits = _allUnits + (crew _x);_allVehicles = _allVehicles + [_x]} foreach _foo;
...
call {
			...
			// Avoid game logics
			_foo = [];
			{if(side _x != sideLogic) then {_foo set [(count _foo) ,_x]}} foreach KEGsAllUnits;
			
			{_allUnits set [(count _allUnits) ,(crew _x) select 0];_allVehicles set [(count _allVehicles), _x]} foreach _foo;
			_newUnits = _allUnits - deathCam;	
			_newVehicles = _allVehicles - _ehVehicles;
			if(count _newVehicles > 0) then {
				_debugPlayer globalchat "~New Vehicles";
